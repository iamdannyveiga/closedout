...
# this script. CLOSEDOUT_API_KEY is not used on this path; the CLI carries its own
# credentials.
#
# CLOSEDOUT_MODEL names the worker model, for example gpt-5. Leave it unset to use
# the default below.
#
...
codex exec -m "$CLOSEDOUT_MODEL" -o "$out" "$prompt"
