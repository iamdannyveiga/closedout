...
# that runs this script. CLOSEDOUT_API_KEY is not used on this path; the CLI
# carries its own credentials.
#
# CLOSEDOUT_MODEL names the worker model, for example claude-sonnet-5. Leave it
...
claude -p --model "$CLOSEDOUT_MODEL" < "$brief" > "$out"
